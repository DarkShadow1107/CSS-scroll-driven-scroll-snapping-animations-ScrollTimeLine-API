var __defProp = Object.defineProperty,
	__defNormalProp = (e, t, n) =>
		t in e ? __defProp(e, t, { enumerable: !0, configurable: !0, writable: !0, value: n }) : (e[t] = n),
...
!(function () {
	...
	class r extends e {
		constructor(e, t = "unrestricted") {
			super(), __publicField(this, "type"), __publicField(this, "value"), (this.value = e), (this.type = t);
		}
	}
	class o extends e {
		constructor(e) {
	...
	class u extends e {
		constructor(e, t = "integer") {
			super(), __publicField(this, "value"), __publicField(this, "type"), (this.value = e), (this.type = t);
		}
	}
	class m extends e {
		constructor(e) {
	...
	function _(e, t, n) {
		return 45 === e ? R(t) || 45 === t || V(t, n) : !!R(e) || (92 === e && V(e, t));
	}
	function L(e, t, n) {
		return 43 === e || 45 === e ? P(t) || (46 === t && P(n)) : P(46 === e ? t : e);
	}
	function O(e) {
		const t = e.consume();
		if (I(t)) {
	...
	function W(e) {
		let t = (function (e) {
			let t = "integer",
				n = "";
			for ([43, 45].includes(e.peek()[0]) && (n += String.fromCodePoint(e.consume())); P(...e.peek()); )
				n += String.fromCodePoint(e.consume());
			if (46 === e.peek()[0] && P(e.peek()[1]))
				for (n += String.fromCodePoint(e.consume(), e.consume()), t = "number"; P(...e.peek()); )
					n += String.fromCodePoint(e.consume());
			return (
				[69, 101].includes(e.peek()[0]) &&
					([45, 43].includes(e.peek()[1]) && P(e.peek()[2])
						? ((n += String.fromCodePoint(e.consume(), e.consume(), e.consume())), (t = "number"))
						: P(e.peek()[1]) && ((n += String.fromCodePoint(e.consume(), e.consume())), (t = "number"))),
	...
	function D(e) {
		...
		if (i.match(/url/i) && 40 === e.peek()[0]) {
			...
			return [34, 39].includes(e.peek()[0]) || (M(e.peek()[0]) && [34, 39].includes(e.peek()[1]))
				? new n(i)
				: (function (e) {
						const t = new a("");
						...
						for (;;) {
							...
							if (M(n)) {
								for (; M(...e.peek()); ) e.consume();
								return 41 === e.peek()[0] || void 0 === e.peek()[0] ? (e.consume(), t) : (F(e), new l());
							}
	...
	function q(e, t) {
		...
		if (!t) throw new Error("To determine the normalized axis the computedStyle of the source is required.");
	...
	function Q(e, t = {}) {
		...
		if (e instanceof CSSMathMin || e instanceof CSSMathMax) {
			...
			if (t.every((e) => e instanceof CSSUnitValue && "percent" !== e.unit && $(e.unit) && e.unit === t[0].unit)) {
		...
		if (e instanceof CSSMathMin || e instanceof CSSMathMax) {
			...
			if (r.some((e) => e.length > 0)) {
				...
				e = e instanceof CSSMathMin ? new CSSMathMin(...t, ...i) : new CSSMathMax(...t, ...i);
			}
		...
		if (e instanceof CSSMathProduct) {
			let t = [];
			for (const r of e.values) r instanceof CSSMathProduct ? t.push(...r.values) : t.push(r);
			const [n, i] = G(t, (e) => e instanceof CSSUnitValue && "number" === e.unit);
			if (n.length > 1) {
			...
			if (2 === t.length) {
				...
				for (const i of t)
					i instanceof CSSUnitValue && "number" === i.unit
						? (e = i)
						: i instanceof CSSMathSum && [...i.values].every((e) => e instanceof CSSUnitValue) && (n = i);
				if (e && n) return new CSSMathSum(...[...n.values].map((t) => new CSSUnitValue(t.value * e.value, t.unit)));
			}
			if (
				t.every(
					(e) =>
						(e instanceof CSSUnitValue && $(e.unit)) ||
						(e instanceof CSSMathInvert && e.value instanceof CSSUnitValue && $(e.value.unit))
				)
			) {
	...
	const X = null,
		Y = ["percent", "length", "angle", "time", "frequency", "resolution", "flex"],
		J = {
			fontRelativeLengths: {
			...
			absoluteLengths: {
				...
				compatible: !0,
				canonicalUnit: "px",
				ratios: { cm: 96 / 2.54, mm: 96 / 2.54 / 10, Q: 96 / 2.54 / 40, in: 96, pc: 16, pt: 96 / 72, px: 1 },
			},
			angle: {
			...
			time: { units: new Set(["s", "ms"]), compatible: !0, canonicalUnit: "s", ratios: { s: 1, ms: 0.001 } },
			frequency: { units: new Set(["hz", "khz"]), compatible: !0, canonicalUnit: "hz", ratios: { hz: 1, khz: 1e3 } },
			resolution: {
		...
		Z = new Map();
	for (const Vt of Object.values(J)) if (Vt.compatible) for (const e of Vt.units) Z.set(e, Vt);
	function ee(e) {
		return Z.get(e);
	}
	function te(e, t) {
	...
	function ne(e) {
		return "number" === e
			...
			: "percent" === e
			? { percent: 1 }
			: J.absoluteLengths.units.has(e) || J.fontRelativeLengths.units.has(e) || J.viewportRelativeLengths.units.has(e)
			? { length: 1 }
			: J.angle.units.has(e)
			? { angle: 1 }
			: J.time.units.has(e)
	...
	function ie(e) {
		if (e instanceof CSSUnitValue) {
			...
			const i = ee(e.unit);
			return (
				i && t !== i.canonicalUnit && ((n *= i.ratios[t]), (t = i.canonicalUnit)),
				"number" === t ? [[n, {}]] : [[n, { [t]: 1 }]]
			);
		}
		if (e instanceof CSSMathInvert) {
			if (!(e.value instanceof CSSUnitValue)) throw new Error("Not implemented");
			const t = ie(e.value);
			if (t === X) return X;
		...
		if (e instanceof CSSMathProduct) {
			...
			for (const n of e.values) {
				const e = ie(n),
					i = [];
				if (e === X) return X;
				for (const n of t) for (const t of e) i.push([n[0] * t[0], te(n[1], t[1])]);
				t = i;
			}
			return t;
		}
	...
	function re(e, t) {
		if (ne(t) === X) throw new SyntaxError("The string did not match the expected pattern.");
		const n = ie(e);
	...
	function ae(e, t) {
		if (e.percentHint && t.percentHint && e.percentHint !== t.percentHint) return X;
	...
	class CSSFunction {
		constructor(e, t) {
			__publicField(this, "name"), __publicField(this, "values"), (this.name = e), (this.values = t);
		}
	}
	class CSSSimpleBlock {
		constructor(e, t) {
			__publicField(this, "value"), __publicField(this, "associatedToken"), (this.value = e), (this.associatedToken = t);
		}
	}
	function le(e) {
	...
	function he(e) {
		if ("ADDITION" === e.type) return new CSSMathSum(...e.values.map((e) => he(e)));
		if ("MULTIPLICATION" === e.type) return new CSSMathProduct(...e.values.map((e) => he(e)));
		if ("NEGATE" === e.type) return new CSSMathNegate(he(e.value));
		if ("INVERT" === e.type) return new CSSMathInvert(he(e.value));
		if (e instanceof CSSSimpleBlock) return pe(new CSSFunction("calc", e.value));
		if (e instanceof t) {
			if ("e" === e.value) return new CSSUnitValue(Math.E, "number");
			if ("pi" === e.value) return new CSSUnitValue(Math.PI, "number");
	...
	function pe(e) {
		if ("min" === e.name || "max" === e.name) {
			const t = e.values.filter((e) => !(e instanceof h || e instanceof v)).map((e) => Q(pe(new CSSFunction("calc", e))));
	...
	function de(e) {
		return e instanceof CSSFunction && ["calc", "min", "max", "clamp"].includes(e.name)
			? pe(e)
	...
	!(function () {
		...
		function t(e) {
			const t = [];
			for (let i = 0; i < e.length; i++) t[i] = "number" == typeof (n = e[i]) ? new CSSUnitValue(n, "number") : n;
			var n;
			return t;
		}
	...
	function Pe(e, t) {
		...
		return { top: n, left: i, offsetWidth: t.offsetWidth, offsetHeight: t.offsetHeight, fontSize: s.fontSize };
	}
	function Ie(e) {
		let t = Te.get(e);
		...
		if (i != t) {
			if (i) {
				...
				if (t) {
					...
					const n = Array.from(t.timelineRefs).filter((e) => void 0 === e.deref());
			...
			if (((n.source = t), t)) {
				...
				if (!i) {
					...
					const o = () => {
						(i.sourceMeasurements.scrollLeft = t.scrollLeft), (i.sourceMeasurements.scrollTop = t.scrollTop);
	...
	class ScrollTimeline {
		constructor(e) {
			...
			if (
				(Re(this, e && void 0 !== e.source ? e.source : document.scrollingElement),
		...
		get phase() {
			...
			return "none" == t.display
				? "inactive"
				: e == document.scrollingElement || ("visible" != t.overflow && "clip" != t.overflow)
				? "active"
				: "inactive";
		}
		get currentTime() {
			...
			const i = this.axis,
				...
				o = (function (e, t) {
					...
					return (
						"block" === t ? (t = i ? "y" : "x") : "inline" === t && (t = i ? "x" : "y"),
						"y" === t ? n.scrollHeight - n.clientHeight : "x" === t ? n.scrollWidth - n.clientWidth : void 0
					);
				})(t, i);
		...
		if (e && e.isConnected) {
			for (; (e = je(e)); ) {
				switch (getComputedStyle(e)["overflow-x"]) {
					...
					case "scroll":
					case "hidden":
						return e == document.body && "visible" == getComputedStyle(document.scrollingElement).overflow
	...
	function Fe(e, t) {
		...
		return "inactive" === e.phase ? null : e instanceof $e ? De(t, r, i, n.axis, n.inset) : null;
	}
	function De(e, t, n, i, r) {
		...
		"x" === q(i, t)
			...
			o && ((a += t.scrollWidth - t.clientWidth), (l.scrollPadding = [t.scrollPaddingRight, t.scrollPaddingLeft])),
		...
		const c = (function (e, t) {
				...
				const [i, r] = [e.start, e.end].map((e, n) =>
					"auto" === e
						...
						: Ce(e, { percentageReference: CSS.px(t.containerSize), fontSize: CSS.px(parseFloat(t.fontSize)) })
				);
	...
	function ze(e, t) {
		...
		if (e instanceof ScrollTimeline) {
			...
			let o;
			o = "x" === q(n, r) ? r.scrollWidth - r.clientWidth : r.scrollHeight - r.clientHeight;
	...
	function He(e, t, n, i) {
		...
		return (
			(Ce(t, { percentageReference: CSS.px(e.end - e.start), fontSize: CSS.px(parseFloat(r.fontSize)) }) +
				e.start -
				n.start) /
			(n.end - n.start)
		);
				...
				e &&
					...
					(t.inset = (function (e) {
						...
						for (const n of t) {
							if ("auto" === n) continue;
							const e = n.type();
							if (1 !== e.length && 1 !== e.percent) throw TypeError("Invalid inset");
						}
	...
	function Je(e, t) {
		if (null === t) return t;
		if ("number" != typeof t)
			throw new DOMException(`Unexpected value: ${t}.  Cannot convert to CssNumberish`, "InvalidStateError");
	...
	function tt(e) {
		const t = Ze(e, e.timeline.currentTime);
		null != e.startTime && null == e.holdTime && (e.holdTime = (t - e.startTime) * e.animation.playbackRate),
			rt(e),
			(e.startTime = null),
	...
	function st(e, t, n) {
		...
		"finished" == e.proxy.playState
			...
			: (e.finishedPromise && "resolved" == e.finishedPromise.state && (e.finishedPromise = new Qe()),
	...
	function ct(e, t) {
		const n = e.timeline,
			...
			r = n.currentTime && n.currentTime.value == (i < 0 ? 0 : 100) ? (i < 0 ? 0.001 : -0.001) : 0;
	...
	function ut(e, t) {
		...
		0 == it(e) && null == r && (e.holdTime = 0),
			...
			("finished" === e.proxy.playState || n) && ((e.holdTime = null), (e.startTime = null), (e.autoAlignStartTime = !0)),
			e.holdTime && (e.startTime = null),
			e.pendingTask && ((e.pendingTask = null), (i = !0)),
			(null !== e.holdTime || e.autoAlignStartTime || n || null !== e.pendingPlaybackRate) &&
	...
	function mt(e) {
		const t = ht.get(this);
		if (!t) return;
		if (null == e) return void ("paused" !== t.proxy.playState && "idle" != t.animation.playState && t.animation.cancel());
		dt(t),
			t.pendingTask &&
				requestAnimationFrame(() => {
	...
	function dt(e) {
		...
		if ("idle" === e.proxy.playState || ("paused" === e.proxy.playState && null !== e.holdTime)) return;
			...
			n = CSS.percent(
				100 *
					(function (e) {
						...
						const t = "normal" === e.animationRange.start ? gt(e.timeline) : e.animationRange.start;
						return ze(e.timeline, t);
					})(e)
			);
		} catch (o) {
			(n = CSS.percent(0)), (e.animationRange.start = "normal"), console.warn("Exception when calculating start offset", o);
		}
		try {
			i = CSS.percent(
				100 *
					(1 -
						(function (e) {
							if (!e.animationRange) return 0;
							const t = "normal" === e.animationRange.end ? vt(e.timeline) : e.animationRange.end;
							return 1 - ze(e.timeline, t);
						})(e))
			);
		} catch (o) {
			(i = CSS.percent(100)), (e.animationRange.end = "normal"), console.warn("Exception when calculating end offset", o);
		}
	...
	function Tt(e, t) {
		...
		if (e instanceof ViewTimeline) {
			...
			return (
				i.length && ((n.start.rangeName = i[0]), (n.end.rangeName = i.length > 1 ? i[1] : i[0])),
	...
	function yt(e, t, n) {
		...
		if (e instanceof ViewTimeline) {
			...
			if (t instanceof Object) void 0 !== t.rangeName && (e = t.rangeName), void 0 !== t.offset && (i = t.offset);
			else {
				const n = B(t);
				1 === n.length
					? ye.includes(n[0])
					...
					: 2 === n.length && ((e = n[0]), (i = Q(CSSNumericValue.parse(n[1]), {})));
			}
	...
	class wt {
							...
							const t = e.animation.effect,
								...
								r = {
									apply: function (i, r) {
										...
										return (
											(null === s.duration || "auto" === s.duration || e.autoDurationEffect) &&
												e.timeline &&
												((e.autoDurationEffect = !0),
												(s.delay = 0),
												(s.endDelay = 0),
												(o = s.iterations ? 1e5 : 0),
												(s.duration = s.iterations ? (o - s.delay - s.endDelay) / s.iterations : 0),
												s.duration < 0 && ((s.duration = 0), (s.endDelay = o - s.delay)),
												n.apply(t, [s])),
		...
		set startTime(e) {
			const t = ht.get(this);
			if (((e = Ze(t, e)), !t.timeline)) return void (t.animation.startTime = e);
			...
			rt(t),
				(t.startTime = e),
				null !== t.startTime && 0 != t.animation.playbackRate ? (t.holdTime = null) : (t.holdTime = n),
		...
		get currentTime() {
			const e = ht.get(this);
			return e.timeline ? (null != e.holdTime ? Je(e, e.holdTime) : Je(e, ot(e))) : e.animation.currentTime;
		}
		set currentTime(e) {
			const t = ht.get(this);
			t.timeline
		...
		get rangeStart() {
			var e;
			return (null == (e = ht.get(this).animationRange) ? void 0 : e.start) ?? "normal";
		}
		set rangeStart(e) {
			const t = ht.get(this);
		...
		get rangeEnd() {
			var e;
			return (null == (e = ht.get(this).animationRange) ? void 0 : e.end) ?? "normal";
		}
		set rangeEnd(e) {
			const t = ht.get(this);
		...
		reverse() {
			const e = ht.get(this),
				...
				i = at(e) == 1 / 0,
				r = 0 != t && (t < 0 || n > 0 || !i);
			if (!e.timeline || !r) return r && (e.pendingPlaybackRate = -it(e)), void e.animation.reverse();
			...
			if (!t.readyPromise || "pending" != t.readyPromise.state)
				switch (n) {
					...
					case "finished":
						...
						(t.startTime = 0 == e ? n : null != n && null != i ? (n - i) / e : null), rt(t), st(t, !1, !1), lt(t);
						break;
					default:
						ut(t);
				}
		...
		cancel() {
			...
			e.timeline
				? ("idle" != this.playState &&
						...
						e.finishedPromise && "pending" == e.finishedPromise.state && e.finishedPromise.reject(Ye()),
		...
		get finished() {
			const e = ht.get(this);
			return e.timeline
				? (e.finishedPromise || (e.finishedPromise = new Qe()), e.finishedPromise.promise)
				: e.animation.finished;
		}
		get ready() {
			const e = ht.get(this);
			return e.timeline
				? (e.readyPromise || ((e.readyPromise = new Qe()), e.readyPromise.resolve(this)), e.readyPromise.promise)
				: e.animation.ready;
		}
	...
	function xt(e, t) {
		...
		if (n instanceof ScrollTimeline) {
			i.pause();
			(ht.get(r).animationRange = { start: yt(n, t.rangeStart, "start"), end: yt(n, t.rangeEnd, "end") }), r.play();
		}
		return r;
	}
	function bt(e) {
	...
	const It = new (class {
		...
		transpileStyleSheet(e, t, n) {
			...
			for (; i.index < i.sheetSrc.length && (this.eatWhitespace(i), !(i.index >= i.sheetSrc.length)); ) {
				if (this.lookAhead("/*", i)) {
					for (; this.lookAhead("/*", i); ) this.eatComment(i), this.eatWhitespace(i);
					continue;
				}
				const e = this.parseQualifiedRule(i);
				e && (t ? this.parseKeyframesAndSaveNameMapping(e, i) : this.handleScrollTimelineProps(e, i));
			}
			return i.sheetSrc;
		}
			...
			for (let n = this.cssRulesWithTimelineName.length - 1; n >= 0; n--) {
				...
				try {
					if (t.matches(i.selector) && (!i["animation-name"] || i["animation-name"] == e))
						return { "animation-timeline": i["animation-timeline"], "animation-range": i["animation-range"] };
				} catch {}
			}
			return null;
		}
		...
		getScrollTimelineOptions(e, t) {
			...
			for (let i = this.sourceSelectorToScrollTimeline.length - 1; i >= 0; i--) {
				...
				if (n.name == e) {
					const e = this.findPreviousSiblingOrAncestorMatchingSelector(t, n.selector);
		...
		getAnonymousViewTimelineOptions(e, t) {
			...
			return n ? { subject: t, axis: n.axis ? n.axis : "block", inset: n.inset ? n.inset : "auto" } : null;
		}
		...
		handleScrollTimelineProps(e, t) {
			...
			if ((this.saveSourceSelectorToScrollTimeline(e), this.saveSubjectSelectorToViewTimeline(e), !i && !n && !r)) return;
			let o = [],
				s = [],
				a = !1;
					...
					(r &&
						this.extractMatches(e.block.contents, kt.ANIMATION).forEach((t) => {
							...
							n && i && s.push(n),
								i &&
									(this.hasDuration(t) ||
										(this.hasAutoDuration(t) && (e.block.contents = e.block.contents.replace("auto", "    ")),
		...
		saveSourceSelectorToScrollTimeline(e) {
			...
			if (t) {
				...
				for (const n of t) {
					...
					1 == t.length
						? (i.name = t[0])
						: 2 == t.length &&
						    (Mt.includes(t[0]) ? ((i.axis = t[0]), (i.name = t[1])) : ((i.axis = t[1]), (i.name = t[0]))),
						r.push(i);
				}
			}
			if (n) {
				const t = this.extractMatches(e.block.contents, kt.SCROLL_TIMELINE_NAME);
			...
			let o = [];
			if (i) {
				const t = this.extractMatches(e.block.contents, kt.SCROLL_TIMELINE_AXIS);
				if (((o = t.filter((e) => Mt.includes(e))), o.length != t.length)) throw new Error("Invalid axis");
			}
			for (let s = 0; s < r.length; s++) o.length && (r[s].axis = o[s % r.length]);
		...
		saveSubjectSelectorToViewTimeline(e) {
			...
			let s = [],
				a = [];
			if ((r && (s = this.extractMatches(e.block.contents, kt.VIEW_TIMELINE_INSET)), i)) {
				const t = this.extractMatches(e.block.contents, kt.VIEW_TIMELINE_AXIS);
				if (((a = t.filter((e) => Mt.includes(e))), a.length != t.length)) throw new Error("Invalid axis");
			}
		...
		saveRelationInList(e, t, n) {
			let i = [];
			e.block.contents.includes("animation-range:") && (i = this.extractMatches(e.block.contents, kt.ANIMATION_TIME_RANGE));
		...
		parseAnonymousViewTimeline(e) {
			...
			return (
				n.split(" ").forEach((e) => {
					Mt.includes(e) ? (i.axis = e) : (i.inset = i.inset ? `${i.inset} ${e}` : e);
				}),
				i
			);
		}
		...
		replaceKeyframesAndGetMapping(e, t) {
			...
			for (let l = 0; l < r.length; l++) {
				...
				e.split(",").forEach((e) => {
					...
					a.push(t.join(",")),
					l == r.length - 1 ? a.push(i.substring(r[l].end)) : a.push(i.substring(r[l].end, r[l + 1].start));
			}
			return s
		...
		parseQualifiedRule(e) {
			...
			if (!n) return;
			return { selector: n, block: this.eatBlock(e), startIndex: t, endIndex: e.index };
		}
		...
		replacePart(e, t, n, i) {
			if (((i.sheetSrc = i.sheetSrc.slice(0, e) + n + i.sheetSrc.slice(t)), i.index >= t)) {
				const r = i.index - t;
		...
		eatUntil(e, t, n = !1) {
			...
			return (
				n && (t.sheetSrc = t.sheetSrc.slice(0, i) + " ".repeat(t.index - i) + t.sheetSrc.slice(t.index)),
	...
	function Nt(e, t, n) {
		...
		if (!r) return null;
		let o = It.getScrollTimelineOptions(r, n) || It.getViewTimelineOptions(r, n);
		return o
			? (o.subject &&
					(function (e, t) {
								...
								if (s == 100 * r.offset) {
									if ("from" == a) o = 0;
										...
										o =
											1 == e.length
												? parseFloat(e[0])
												: 100 * Rt(e[0], n, t.subject, i, t.inset, CSS.percent(parseFloat(e[1])));
									}
									break;
								}
							return o;
						...
						if (o && o.size) {
							...
							e.effect.getKeyframes().forEach((e) => {
								const n = r(o, e);
								null !== n && n >= 0 && n <= 100 && ((e.offset = n / 100), t.push(e));
							});
							const n = t.sort((e, t) => (e.offset < t.offset ? -1 : e.affset > t.offset ? 1 : 0));
							e.effect.setKeyframes(n);
						}
					})(e, o),
			    { timeline: o.source ? new ScrollTimeline(o) : new $e(o), animOptions: i })
			: null;
	}
	function At() {
		...
		!(function () {
			...
			new MutationObserver((n) => {
				for (const i of n)
					for (const n of i.addedNodes) n instanceof HTMLStyleElement && e(n), n instanceof HTMLLinkElement && t(n);
